...
'Constants
Const cLetterWidth = 6
Const cLetterHeight = 9
Const cFirstChar = 32       ' Space
Private iLastChar As Integer
...
'WARNING! DO NOT REMOVE OR MODIFY THE FOLLOWING COMMENTED LINES!
'MappingInfo=imgLetters,imgLetters,-1,Picture
Public Property Get Charset() As Picture
Attribute Charset.VB_ProcData.VB_Invoke_Property = ";Appearance"
    Set Charset = imgLetters.Picture
End Property

Public Property Set Charset(ByVal New_Charset As Picture)
    If New_Charset Is Nothing Then
        Set imgLetters.Picture = imgSpace.Picture
    Else
        Set imgLetters.Picture = New_Charset
    End If
    GenerateMemImgDC imgDC, imgBitmap, imgLetters
    iLastChar = ((imgLetters.Width - 1) / cLetterWidth) + cFirstChar - 1
    DrawControl
...
'WARNING! DO NOT REMOVE OR MODIFY THE FOLLOWING COMMENTED LINES!
'MemberInfo=11,0,0,0
Public Property Get Drawing() As Picture
Attribute Drawing.VB_ProcData.VB_Invoke_Property = ";Appearance"
    Set Drawing = m_Drawing
End Property

Public Property Set Drawing(ByVal New_Drawing As Picture)
'    If m_Mode = AlphaNumeric Then Exit Property
    Set m_Drawing = New_Drawing
    DrawControl
    PropertyChanged "Drawing"
End Property
        ...
        Case AlphaNumeric
            ...
            For Y = 0 To m_TotalRows - 1
                For X = 0 To m_TotalCols - 1
                    BitBlt hDC, cLetterWidth * X, cLetterHeight * Y, cLetterWidth + 1, cLetterHeight, imgDC, 0, 0, vbSrcCopy
                Next
            Next
            ReDim WriteArray(m_TotalRows - 1): For X = 0 To m_TotalRows - 1: WriteArray(X) = Space(m_TotalCols): Next
        Case Graphic
            Set UserControl.Picture = m_Drawing
            UserControl_Resize
...
On Error GoTo GenerateMemImgDCError
...
Private Function ScrollLines()
    ...
    If Not tmrWrite.Enabled Then
        ...
        For Y = 0 To m_TotalRows - 1
            For X = 0 To m_TotalCols - 1
                BitBlt hDC, cLetterWidth * X, cLetterHeight * Y, cLetterWidth + 1, cLetterHeight, imgDC, cLetterWidth * (Asc(Mid(WriteArray(Y), X + 1, 1)) - 32), 0, vbSrcCopy
            Next
        Next
        Refresh
...
Private Sub TextFixAlign(ByRef sOldText As String, sNewText As String, ByVal Align As Integer, ByVal iCol As Integer)
    ...
    For i = 1 To Len(sNewText)
        ...
        If (Asc(sTemp) < cFirstChar) Or (Asc(sTemp) > iLastChar) Then
            sNewText = Replace(sNewText, sTemp, " ")
        End If
    Next
    i = m_TotalCols - Len(sNewText)
    iSpcSize = IIf(i < 0, 0, i)
    sNewText = Left(sNewText, m_TotalCols - iCol + 1)
    Select Case Align
        Case LCDAlignLeft
            sNewText = Left(sOldText, iCol - 1) & sNewText
            If (iSpcSize - iCol + 1) > 0 Then
                sOldText = sNewText & Right(sOldText, iSpcSize - iCol + 1)
            Else
                sOldText = sNewText
            End If
        Case LCDAlignCenter
...
Public Function WriteText(ByVal sText As String, Optional ByVal Align As Integer = 0, Optional ByVal iRow As Integer = 0, Optional ByVal iCol As Integer = 1)
    ...
    If m_WriteSpeed = 0 Then
        ...
        For X = 0 To m_TotalCols - 1
            BitBlt hDC, cLetterWidth * X, cLetterHeight * (iRowToWrite - 1), cLetterWidth, cLetterHeight, imgDC, cLetterWidth * (Asc(Mid(sText, X + 1, 1)) - 32), 0, vbSrcCopy
        Next
        Refresh
...
Private Sub UserControl_ReadProperties(PropBag As PropertyBag)
    ...
    Set imgLetters.Picture = PropBag.ReadProperty("Charset", Nothing)
    Set m_Drawing = PropBag.ReadProperty("Drawing", Nothing)
...
Private Sub UserControl_Resize()
    ...
    bResizing = True
    Width = (m_TotalCols * cLetterWidth + 1) * Screen.TwipsPerPixelX
    Height = m_TotalRows * cLetterHeight * Screen.TwipsPerPixelY
    Select Case m_Mode
        Case AlphaNumeric
        Case Graphic
            If m_AutoSize And Not (m_Drawing Is Nothing) Then
                Call GetObject(m_Drawing, Len(PicInfo), PicInfo)
...
Private Sub UserControl_WriteProperties(PropBag As PropertyBag)
    ...
    Call PropBag.WriteProperty("Charset", imgLetters.Picture, Nothing)
    Call PropBag.WriteProperty("Drawing", m_Drawing, Nothing)
...
Private Sub tmrWrite_Timer()
    ...
    If iTextStart >= iTextStop Then
        ...
        Exit Sub
    End If
    BitBlt hDC, cLetterWidth * (iTextStart - 1), cLetterHeight * (iRowToWrite - 1), cLetterWidth, cLetterHeight, imgDC, cLetterWidth * (Asc(Mid(sTextToWrite, iTextStart - iTemp, 1)) - 32), 0, vbSrcCopy
